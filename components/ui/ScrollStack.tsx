...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  // Cached offsets — read once on mount/resize, not on every scroll frame
  ...
  // Reads the DOM once; call on mount and resize only, never inside scroll handler
  const cacheOffsets = useCallback(() => {
    ...
    if (useWindowScroll) {
      ...
      endElementOffsetRef.current = endEl ? endEl.offsetTop : 0;
    }
  }, [useWindowScroll]);
  ...
  useLayoutEffect(() => {
    ...
    // Cache offsets once after layout, then again on resize
    cacheOffsets();
